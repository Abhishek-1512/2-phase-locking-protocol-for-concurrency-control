...
with open('input4.txt','r') as fp:  #reading the input file
            ...
            if len(lock_table)==0: #if no data_items present in the Lock_table
                ...
                for i in range(len(lock_table)):#if Data_item present
                    ...
                    if lock_table[i][0]==Data_item:
                        ...
                        #then wait die protocol is implemented by comparing the timestamps of Transactions.
                        elif lock_table[index_lt][1]=='w' and Transaction_id not in lock_table[index_lt][2]:#wait die protocol
                            ...
                            if(timestamp1>timestamp2): # if requesting transition is younger ...kill it.
                                print("Abort T"+str(t1)+" as it is younger than T"+str(t2))
                                output_file.write("Abort T"+str(t1)+" as it is younger than T"+str(t2))
                                output_file.write('\n')
                                Abort_transaction(t1)
                 
                            elif(timestamp1<timestamp2):#if requesting transaction is older, block it.
                                print("BLOCK T"+str(t1)+" as ITEM "+str(Data_item)+" is held by T"+str(t2)+" and T"+str(t1)+" is older than T"+str(t2))
                                output_file.write("BLOCK T"+str(t1)+" as ITEM "+str(Data_item)+" is held by T"+str(t2)+" and T"+str(t1)+" is older than T"+str(t2))
                                output_file.write('\n')
                    ...
                    if lock_table[i][0]==Data_item:
                        ...
                        if lock_table[index_lt][1]=='w' and Transaction_id in lock_table[index_lt][2]:
                            ...
                            for sublist in Transaction_table:
                                    ...
                                    if(sublist[0]==t2):
                                        timestamp2=sublist[1]
                            #comparing timestamps to implement wait die protocol
                            if(timestamp1>timestamp2):# if requesting transition is younger ...kill it.
                                print("Abort T"+str(t1)+" as it is younger than T"+str(t2))
                                output_file.write("Abort T"+str(t1)+" as it is younger than T"+str(t2))
                                output_file.write('\n')
                            ...
                            if len(lock_table[index_lt][2])==1:#checking only one Transaction has read lock
                                ...
                                for sublist in Transaction_table:#iterrate through for loop to compare timestamps to implement wait die protocol
                                    if(sublist[0]==t1):
                                        timestamp1=sublist[1]
                                ...
                                for lock_table_tid in lock_table[index_lt][2]:
                                    if(lock_table_tid!=Transaction_id):
                                        ...
                                        if(timestamp1>timestamp2):# if requesting transition is younger ...kill it.
                                            ...
                                            # Update the data item,lock mode and tid of older transaction
                                            for sublist in Transaction_table:
                                                if(sublist[0]==t1 and sublist[2]=='active'):
    ...
    for i in range(len(Transaction_table)):
        if Transaction_table[i][0] == blocked_tid:
            if Transaction_table[i][2] == 'active':
                ...
                Transaction_table[i][3] = blocked_by_tid
...
def Abort_transaction(abort_tid):
    ...
    for i in range(len(Transaction_table)):
        ...
        if Transaction_table[i][3]==abort_tid:
